...
if __name__ == "__main__":
    ...
    if sys.argv[1].isdecimal():
        ...
        in_img_path = pathlib.Path(f'generate-{n_colors_or_rgb_arr}.png')
        c = 0
    else:
        in_img_path = pathlib.Path(sys.argv[1])
        assert in_img_path.is_file()

    if in_img_path.is_file():
    ...
    while (C is None or c < C):
        ...
        timestamp = datetime.datetime.now().isoformat().split(".")[0].replace("-", "").replace("T", "").replace(":", "")

        if out_img_path.is_file():
            out_img_bak_path = out_img_path.with_suffix(f'.bak-{timestamp}' + out_img_path.suffix)
        ...
        if report_str_path.is_file():
            report_str_bak_path = report_str_path.with_suffix(f'.bak-{timestamp}' + report_str_path.suffix)
